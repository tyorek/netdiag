...
2. **`AutoNetworkDiagrammer`** (core scan/graph logic, decoupled from the GUI via `log`/`progress` callbacks so it's usable headlessly):
   ...
   - Device type/color inference (`guess_device_type`, `get_color`) is a flat hostname/IP substring heuristic — extend the existing keyword lists rather than restructuring. Each type also has a hand-drawn SVG glyph in `_DEVICE_ICON_GLYPHS`; nodes are rendered as `shape="image"` with `_device_icon_data_uri()` baking the glyph onto the type's color as a base64 data URI (no external image/font files, so the generated HTML stays self-contained). `_inject_legend()` adds a matching icon legend to the output HTML for whichever types actually appeared. When adding a new device type, update all three: `guess_device_type`, `get_color`, `_DEVICE_ICON_GLYPHS`/`_DEVICE_LABELS`.
...
`APP_VERSION` (top of `network_diagrammer_gui.py`, shown in the window title) follows semver and is bumped alongside a new entry in `CHANGELOG.md` for user-visible changes. Keep `README.md`'s version badge in sync with it.
